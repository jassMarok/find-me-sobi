...
var posTemp;
var infoWindow;
var sobiHubs = [];                          //Object for some procssed info sorted by nearest
              
var travelMode = 'WALKING';                   //Travel mode for distance matrix
function initMap() {
    ...
    var userLocation = geoLocating();        //Origin for distance calculation  
    var latLng=Array();
    latLng=buildArray(appData);
    findNearest(latLng);
...
function findNearest(thisdata) {             //Find nearest point on map
   var userLocation=new google.maps.LatLng(43.255203,-79.843826);
    var distanceArray=Array();                                   
    for(var i=0;i<thisdata.length-1;i++){
              var tempDistance=
        google.maps.geometry.spherical
        .computeDistanceBetween(thisdata[i],userLocation);
        distanceArray.push(tempDistance);
        console.log("Array :"+[i]+" Distance:"+tempDistance);
    }
...
function buildArray(thisJson) {
    var tempArray = new Array();
    var objLength=thisJson.length;
    for (var temp = 0; temp < thisJson.length-1; temp++) {
        var tempLat=parseFloat(thisJson[temp]["LATITUDE"]);
        var tempLng=parseFloat(thisJson[temp]["LONGITUDE"]);
        var tempLatLng = new google.maps.LatLng(tempLat,tempLng);
        //console.log("hi"+tempLatLng);
         tempArray[temp] =tempLatLng;
         console.log("hi"+tempArray[0]);
    }
    return tempArray;
}
